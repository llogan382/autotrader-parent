...
<h2><?php _e('Discuss this article:', 'tfuse'); ?></h2>
<a href="#addcomments" class="link-add-comment anchor"><?php comments_number( 'No Comments Yet', '1 Comment', '% Comments' ); ?></a>
...
$args = array(
    ...
    'comment_field' => ' <div class="row">
							...
							<textarea name="comment" class="textarea textarea_middle required" id="comment" rows="10" cols="30" tabindex="4" ' . $aria_req . '></textarea>
						</div>',
    ...
    'fields' => apply_filters( 'comment_form_default_fields', array(
            ...
            'author' =>'<div class="row alignleft field_text">
                            <label class="label_title" for="author"><strong>'.__('NAME','tfuse').'</strong></label>
                            <input type="text" name="author" class="inputtext input_middle required" id="author" value="' . esc_attr( $commenter['comment_author'] ) .'" tabindex="1" ' . $aria_req . '/>
                        </div>',

            'email' =>'<div class="row alignleft field_text omega">
                            <label class="label_title" for="email"><strong>'.__('EMAIL','tfuse').'</strong> ('.__('never published', 'tfuse').')</label>
