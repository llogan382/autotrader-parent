...
class TF_SLIDER_MODEL extends TF_TFUSE {
    ...
    function save_slides($id, $options = '', $additional = array()) {
        ...
        if (isset($id))
            $this->id = $id;
        $slider_settings = array( 'general' => array(), 'slides' => array() );
        ...
        unset($option_list);
        $slider_settings = $options; // fix for php 7.1
    ...
    function title_exists($title, $id = '') {
        ...
        foreach ($db as $slider_id => $slider)
            if ( isset( $slider['general']['slider_title'] ) && $slider['general']['slider_title'] == $title) {
                if ($slider_id != $id)
                    return TRUE;
            }
        return FALSE;
