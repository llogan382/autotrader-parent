...
add_filter('the_content', 'tfuse_formatter', 99); // for brizy this need to be 9, but then we force users to use [raw] shortcode in many cases, for columns for example this works fine with 99, but with 9 need to put [raw][column_shortcode][/raw]
...
add_action( 'wp_enqueue_scripts', 'fuse_framework_enqueue_scripts_frontend' );
function fuse_framework_enqueue_scripts_frontend() {
	wp_enqueue_style( 'fuse-framework-frontend', TFUSE_ADMIN_CSS . '/frontend.css' );
}
