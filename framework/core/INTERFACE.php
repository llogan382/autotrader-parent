...
class TF_INTERFACE extends TF_TFUSE
{
    ...
    public function admin_add_js()
    {
        ...
        wp_register_script('maps.google.com', '//maps.google.com/maps/api/js?v=3' . tfuse_gmap_key(), array('jquery'), '1.0', true);
        wp_register_script(
            'tfuse-upload',
    ...
    public function create_meta_box($post_type)
    {
        ...
        foreach ($meta_boxes as $box) {
            if ($box['meta_box']['context'] == 'side')
                add_meta_box($box['meta_box']['id'], $this->theme->theme_name . ' - ' . $box['meta_box']['name'], array($this, 'inner_custom_box'), $post_type, $box['meta_box']['context'], $box['meta_box']['priority'], array( 'fuse_options' => $box['rows'] ) );
        ...
        if (count($normal_boxes) > 0) {
            ...
            add_meta_box('tfuse_framework_options_metabox', $this->theme->theme_name . ' - Framework options', array($this, 'inner_custom_box'), $post_type, $box['meta_box']['context'], 'high', array( 'fuse_options' => $content ) );
        }
    }
    ...
    {
		if ( ! isset( $args['args']['fuse_options'] ) ) {
			return;
		}

        static $i = 0;

        echo $args['args']['fuse_options'];
