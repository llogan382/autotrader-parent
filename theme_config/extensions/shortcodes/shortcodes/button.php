...
function tfuse_button($atts, $content = null)
{
    ...
    {
        $class = 'btn button_styled';
        $style = ' style="background-color:'.$bgcolor.';color:'.$textcolor.';border:1px solid '.$bordercolor.';"';
        return '<a href="' . $link . '" class="' . $class . '"  target="' . $target . '"' . $style . '>' . $content . '</a>&nbsp;&nbsp;';
    }
    else{
        $class = 'btn ' . $class;
...
$atts = array(
    ...
    'options' => array(
        array(
            'name' => __('Class','tfuse'),
            'desc' => __('Specifies one or more class names for an shortcode, separated by space.<br /><b>predefined classes:</b> btn_pink, btn_black, btn_blue, btn_yellow, btn_green','tfuse'),
